...
static void clipboard_read(void)
{
    ...
    if (to_amiga) {
        ...
        if (warn_once) return;
        write_log(_T("clipboard: read host clipboard but ignored because previous clip transfer still active\n"));
        warn_once = 1;
#endif
        return;
    }
    clipboard_change = 0;
#if DEBUG_CLIP > 0
    // write_log(_T("clipboard: read host clipboard\n"));
#endif


    char *new_text = SDL_GetClipboardText();
    if (new_text) {
        if (last_text && strcmp(new_text, last_text) == 0) {
            /* Unchanged */
        } else {
            text = TRUE;
        }
...
UAE_EXTERN_C void uae_clipboard_update(void)
{
    ...
    if (clipboard_to_host_text) {
        write_log("[CLIPBOARD] Set host clipboard to \"%s\"\n",
                  clipboard_to_host_text);
        SDL_SetClipboardText(clipboard_to_host_text);
        free(clipboard_to_host_text);
        clipboard_to_host_text = NULL;
    }
    fs_mutex_unlock(clipboard_mutex);
